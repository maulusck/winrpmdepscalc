...
def download_file_python(url, output_file):

    proxies = urllib.request.getproxies()
    https_proxy = proxies.get("https")

    proxy_dict = {"https": https_proxy} if https_proxy else None

    session = requests.Session()
    session.auth = HttpNegotiateAuth()

    tqdm.write(f"Using system HTTPS proxy: {https_proxy}")

    with session.get(url, stream=True, proxies=proxy_dict, verify=False) as response:
        response.raise_for_status()
        total_size = int(response.headers.get("content-length", 0))
        block_size = 1024

        with open(output_file, "wb") as file, tqdm(
            total=total_size, unit="iB", unit_scale=True, desc=f"Downloading {url}"
        ) as bar:
...
def download_file_python(url, output_file, proxy_url=None):
    """
    Downloads a file from the URL to the output path,
    optionally using the specified HTTPS proxy URL.
    """
    proxies = {}
    if proxy_url:
